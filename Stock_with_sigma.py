...
mu_path_up = np.zeros((steps + 1))
mu_path_down = np.zeros((steps + 1))
for t in range(1, steps + 1):
    mu = (price_matrix[t, :].mean()).item()
    sigma = (price_matrix[t, :].std()).item()
    mu_path_up[t] = mu + 2*sigma
    mu_path_down[t] = mu - 2*sigma
    
plt.plot(future_days,mu_path_up[1:], 'b', lw=3, label='Mu + Sigma') # Plot mu plus sigma path. r-- is red dashed line
plt.plot(future_days,mu_path_down[1:], 'b', lw=3, label='Mu - Sigma') # Plot mu minus sigma path

plt.fill_between(future_days, mu_path_up[1:], mu_path_down[1:], color='red', alpha=0.1)
# Fill area between mu plus sigma and mu minus sigma
